...
import { paymentPromiseSchema } from "@/modules/conversations/occurrence.schemas";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const result = await getOccurrenceService().registerPaymentPromise({
      ...
      request: paymentPromiseSchema.parse(await request.json()),
